#### --------------------------------------------------------------------------------------------------------- ####
### STATE VARIABLE - C108_tree_growing_season_weather_stations
### Date: 07.07.2026
...
## load treeline function from GitHub
...
## check timeOffsets
element_info_timeOffset <- filter(element_info_raw, elementId == elements & timeOffset %in% c("PT0H", "PT6H")) %>% 
...
write.table(aux, paste(temp_dir, "C108_tree_growing_season_weather_stations_aux.txt", sep = "/"), row.names = FALSE, sep = ";")
write.table(coord, paste(temp_dir, "C108_tree_growing_season_weather_stations_coordinates.txt", sep = "/"), row.names = FALSE, sep = ";")
...
for (i in 1:nrow(aux)) {
  ...
  ## format data
  data_list[[i]] <- raw_dat %>% filter(timeOffset %in% c("PT0H", "PT6H")) %>%   
...
## calculate state variable, treeline function is sourced from GitHub
state_var_all <- treeline(data_all)

## save data to a temporary directory
file_name <- paste0("C108_tree_growing_season_weather_stations_", min(state_var_all$t_year), "_", max(state_var_all$t_year), ".txt")
file_name

write.table(state_var_all, file = ".../C108_tree_growing_season_weather_stations_1867_2025.txt", row.names = FALSE, sep = ";")
write.table(aux, file = ".../C108_tree_growing_season_weather_stations_aux.txt", row.names = FALSE, sep = ";")
write.table(coord, file = ".../C108_tree_growing_season_weather_stations_coordinates.txt", row.names = FALSE, sep = ";")
...
state_name <- "C108_tree_growing_season_weather_stations_v1" # write here the name including the version of the state variable you want to add data to
...
resource_create(
  ...
  description = NULL,
  upload = paste(tempdir(), "C108_tree_growing_season_weather_stations_aux.txt", sep = "/"),
  name = "C108_tree_growing_season_weather_stations_aux.txt",
  http_method = "POST"
)
...
resource_create(
  package_id = pkg_state$id,
  description = NULL,
  upload = paste(tempdir(), "C108_tree_growing_season_weather_stations_coordinates.txt", sep = "/"),
  name = "C108_tree_growing_season_weather_stations_coordinates.txt",
  http_method = "POST"
)
