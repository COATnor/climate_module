...
## annual series of daily mean temperature (tg), precipitation amount per day (rr) and daily snow depth (sd)
elements_temp <- "mean(air_temperature P1D)" # daily mean temperature
elements_prec <- "sum(precipitation_amount P1D)" # daily precipitation amount
...
## make the aux file
aux <- station_info %>% 
  mutate(date_last_temperature = NA, date_last_precipitation = NA, date_last_snow = NA, v_comment = NA) %>% 
  select(sn_region, v_station_name, v_station_id, v_owner, v_meter_above_sea_level, date_first_temperature, date_last_temperature, date_last_precipitation, date_last_snow, v_comment)
...
### check daily mean temperature data for the aux file
...
aux$v_station_name[!aux$v_station_id %in% element_info_temp$v_station_id] # add comment if there is no data available
missing_station_temp <- aux$v_station_name[!aux$v_station_id %in% element_info_temp$v_station_id]

## check if there are any gaps
gaps_temp <- element_info_temp %>%
  ...
  filter(gap == TRUE)
gaps_temp  # add comment if there are stations with gaps

## add comments
aux$v_comment[aux$v_station_name %in% missing_station_temp]
aux$v_comment[aux$v_station_name %in% missing_station_temp] <- "no temperature data available from this station"
...
## format data
element_info_prec <- filter(element_info_raw, elementId == elements_prec & timeOffset %in% c("PT6H", "PT18H")) %>% 
  select(sourceId, validFrom, validTo, timeOffset) %>% 
  mutate(sourceId = sub(":0", "", sourceId)) %>%  
  mutate(validFrom = substr(validFrom, 1, 10), validTo = substr(validTo, 1, 10)) %>% 
  rename(v_station_id = sourceId) %>% 
...
## check if temperature data is available for all stations
aux$v_station_name[!aux$v_station_id %in% element_info_prec$v_station_id] # add comment if there is no data available
...
## add comments
no_prec_data_rows <- aux$v_station_name %in% missing_station_prec
no_prec_data_comments <- aux$v_comment[no_prec_data_rows]
comment_no_prec_data <- "no precipitation data available from this station"
aux$v_comment[no_prec_data_rows] <- ifelse(is.na(no_prec_data_comments), comment_no_prec_data, paste(no_prec_data_comments, comment_no_prec_data, sep = ", "))
...
aux <- aux %>%
  ...
  select(sn_region, v_station_name, v_station_id, v_owner, v_meter_above_sea_level, date_first_temperature, date_last_temperature, date_first_precipitation, date_last_precipitation, date_last_snow, v_comment)
...
aux <- aux %>% 
  ...
  select(sn_region, v_station_name, v_station_id, v_owner, v_meter_above_sea_level, date_first_temperature, date_last_temperature, date_first_precipitation, date_last_precipitation, date_first_snow, date_last_snow, v_comment)
...
write.table(aux, paste(temp_dir, "C701_rain_on_snow_days_weather_stations_aux.txt", sep = "/"), row.names = FALSE, sep = ";")
write.table(coord, paste(temp_dir, "C701_rain_on_snow_days_weather_stations_coordinates.txt", sep = "/"), row.names = FALSE, sep = ";")
...
for (i in 1:nrow(aux)) {
  ...
  ## format data
  data_list_temp[[i]] <- raw_dat_temp %>% filter(timeOffset %in% c("PT0H", "PT18H")) %>%   
  ...
  data_list_prec[[i]] <- raw_dat_prec %>% filter(timeOffset %in% c("PT6H", "PT18H")) %>%   
    mutate(t_date = ymd(as.Date(referenceTime))) %>% 
    mutate(t_year = year(t_date), month = month(t_date), day = day(t_date)) %>% 
    mutate(station_id = substr(sourceId, 1, 7)) %>% 
    select(station_id, t_year, t_date, value, timeOffset) %>% 
    rename(v_precipitation_day = value) %>% 
    mutate(sn_region = aux$sn_region[i], v_station_name = aux$v_station_name[i], v_station_id = aux$v_station_id[i]) %>% 
    select(sn_region, v_station_name, v_station_id, t_year, t_date, v_precipitation_day, timeOffset) %>%
    group_by(v_station_id, t_date) %>%
  ...
  ## add missing dates with NA
  first_date_prec <- min(data_list_prec[[i]]$t_date)
  last_date_prec <- max(data_list_prec[[i]]$t_date)
  all_dates_prec <- seq(as.Date(first_date_prec), as.Date(last_date_prec), by = "day")
  missing_dates_prec <- all_dates_prec[!all_dates_prec %in% data_list_prec[[i]]$t_date]
  
  data_list_prec[[i]] <- data_list_prec[[i]] %>% 
    add_row(sn_region = aux$sn_region[i], v_station_name = aux$v_station_name[i], v_station_id = aux$v_station_id[i], t_year = year(missing_dates_prec), t_date = missing_dates_prec, v_precipitation_day = NA) %>% 
    arrange(t_date)
...
data_all_temp <- bind_rows(data_list_temp) %>% select(-timeOffset)
data_all_prec <- bind_rows(data_list_prec) %>% select(-timeOffset)
data_all_snow <- bind_rows(data_list_snow)

check_snow_duplicates <-  data_all_snow %>%
  get_dupes(v_station_id, t_date) 
...
data_all <- data_all_temp %>%
  ...
  full_join(
    data_all_snow,
    by = c("sn_region", "v_station_name", "v_station_id", "t_year", "t_date")) %>%
  arrange(sn_region, v_station_name, t_date)
...
state_var_all <- ros(data_all) %>%
  arrange(sn_region, v_station_name, t_year_winter)

## save data to a temporary directory
file_name <- paste0("C701_rain_on_snow_days_weather_stations_", min(state_var_all$t_year_winter), "_", max(state_var_all$t_year_winter), ".txt")
file_name

write.table(state_var_all, file = ".../C701_rain_on_snow_days_weather_stations_1868_2026.txt", row.names = FALSE, sep = ";")
write.table(aux, file = ".../C701_rain_on_snow_days_weather_stations_aux.txt", row.names = FALSE, sep = ";")
write.table(coord, file = ".../C701_rain_on_snow_days_weather_stations_coordinates.txt", row.names = FALSE, sep = ";")
...
state_name <- "c701_rain_on_snow_days_weather_stations_v1"
...
resource_create(
  ...
  description = NULL,
  upload = paste(tempdir(), "C701_rain_on_snow_days_weather_stations_aux.txt", sep = "/"),
  name = "C701_rain_on_snow_days_weather_stations_aux.txt",
  http_method = "POST"
)
...
resource_create(
  package_id = pkg_state$id,
  description = NULL,
  upload = paste(tempdir(), "C701_rain_on_snow_days_weather_stations_coordinates.txt", sep = "/"),
  name = "C701_rain_on_snow_days_weather_stations_coordinates.txt",
  http_method = "POST"
)
